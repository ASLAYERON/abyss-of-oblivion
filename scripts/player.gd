...
func handle_stamina():
	...
	if stamina < Global.max_stamina && !using_stamina:
		if can_charge_stamina:
			stamina += 1
		elif stamina_charge_timer.time_left == 0:
			stamina_charge_timer.start()
	elif using_stamina:
		can_charge_stamina = false
		stamina_charge_timer.stop()
